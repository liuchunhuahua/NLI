...
def label_input(true_label,pred_label,pred_prob,input_file,output_file):
  '''
  '''
  if len(true_label) != len(pred_label):
    ...
    for i in range (data_num):
      ...
      out_line = "true:"+ str(true_label[i]) + "\tpred:"+ str(pred_label[i]) +"\tprob:"+ str(pred_prob[i])+"\t"+ str(input_txt[i][:-2])+"\t"+ str(input_txt[i][-1])  +"\n"
